...
MODEL_PATH = '/data/wanglinge/project/cav-mae/src/weight/teacher'
_MODELS = {
...
class ClapVisionTransformer(nn.Module):
    def __init__(self, input_resolution, patch_size, width, layers, heads, output_dim, 
                 clip_norm_type='l2', kernel_size=1,
                 return_attn=False, clip_return_layer=1, clip_return_interval=1,
                 clip_return_cls=False, audio_model=None, text_model=None):
        super().__init__()
        ...
        # Visual transformer parts, as in the ViT
        self.conv1 = nn.Conv3d(3, width, (kernel_size, patch_size, patch_size), (kernel_size, patch_size, patch_size), (0, 0, 0), bias=False)
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))
        ...
        self.transformer = Transformer(
            width, layers, heads, return_attn=return_attn, 
            clip_return_layer=clip_return_layer, clip_return_interval=clip_return_interval
        )

        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
        
    def forward(self, x, audio_input=None, text_input=None, mask=None):
        # ViT vision transformer part
        x = self.conv1(x)
        B, C, T, H, W = x.shape
        x = x.permute(0, 2, 3, 4, 1).reshape(B * T, H * W, C)
        ...
        if audio_input is not None:
            audio_features = self.audio_model(audio_input)  # 提取音频特征
        if text_input is not None:
            text_features = self.text_model(text_input)  # 提取文本特征
        ...
        if self.return_attn:
            return x, None  # 如果需要返回 attention，可以加上相关代码
        else:
            return x
