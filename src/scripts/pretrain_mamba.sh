...
exp_dir=./exp/trainmamba-k700-${model}-lr${lr}-bs${batch_size}-norm${norm_pix_loss}-c${contrast_loss_weight}-p${mae_loss_weight}-tp${tr_pos}-mr-${mask_mode}-${masking_ratio}
mkdir -p $exp_dir


PYTHONWARNINGS=ignore torchrun --nproc_per_node=4 pretrain_mamba.py --model ${model} --dataset ${dataset} \
