...
exp_dir=./exp/trainmamba-k700-${model}-lr${lr}-bs${batch_size}-c${contrastive_loss_weight}-p${ts_loss_weight}-m${masking_ratio}
mkdir -p $exp_dir


PYTHONWARNINGS=ignore torchrun --nproc_per_node=5 run_pretrain_mamba.py --model ${model} --dataset ${dataset} \
